...
```{r results='hide', include = F, message=FALSE}
...
CRE_results2 <- BioQCREtoNet(Rels, evidence2, Ents, is.Logfc = TRUE)
# Get FDR corrected p-values
CRE_results2$pvalue <- fdrtool(CRE_results2$pvalue, "pvalue", FALSE, 
                                                      FALSE, FALSE, "fndr")$q
head(CRE_results2[order(CRE_results2$pvalue), c("uid","name","pvalue")])

CRE_results3 <- BioQCREtoNet(Rels, evidence3, Ents, is.Logfc = TRUE)
# Get FDR corrected p-values
CRE_results3$pvalue <- fdrtool(CRE_results3$pvalue, "pvalue", FALSE, 
                                                      FALSE, FALSE, "fndr")$q
head(CRE_results3[order(CRE_results3$pvalue), c("uid","name","pvalue")])
```
