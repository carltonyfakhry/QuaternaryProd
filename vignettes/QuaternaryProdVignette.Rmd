...
\(\indent\) \(\indent\) \(\indent\) \(\indent\) Here we provide functionality for using **QuaternaryProd** with the STRINGdb Homo Sapien causal network provided under the creative commons license. 

### Compute Pvalues Over the Network
\indent  Given new gene expression data, we can compute the scores and p-values for all source nodes in the STRINGdb Homo Sapien causal network using the specialized `RunCRE_HSAStringDB` function.
...
CRE_results <- RunCRE_HSAStringDB(gene_expression_data1, method = "Enrichment",
                                   fc.thresh = log2(1.3), pval.thresh = 0.05)
# Get FDR corrected p-values
CRE_results["qvalue"] <- p.adjust(CRE_results$pvalue, method = "fdr")
# CRE_results[1:400, c("uid","name","regulation","pvalue","qvalue")]


CRE_results2 <- RunCRE_HSAStringDB(gene_expression_data2, method = "Enrichment",
                                   fc.thresh = log2(1.3), pval.thresh = 0.05)
# Get FDR corrected p-values
CRE_results2["qvalue"] <- p.adjust(CRE_results2$pvalue, method = "fdr")
# CRE_results2[1:400, c("uid","name","regulation","pvalue","qvalue")]


CRE_results3 <- RunCRE_HSAStringDB(gene_expression_data3, method = "Enrichment",
                                   fc.thresh = log2(1.3), pval.thresh = 0.05)
# Get FDR corrected p-values
CRE_results3["qvalue"] <- p.adjust(CRE_results3$pvalue, method = "fdr")
# CRE_results3[1:400, c("uid","name","regulation","pvalue","qvalue")]
